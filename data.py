...
if __name__ == '__main__':
    # Test Functionality
    names = ['mnist', 'imdb', 'adult']
    data_fns = [load_mnist, load_imdb, load_adult]
    dummy_data_fns = [load_dummy_mnist, load_dummy_imdb, load_dummy_adult]
    for name, data_fn, dummy_data_fn in zip(names, data_fns, dummy_data_fns):
        ...
        assert y.shape == dy.shape, f'Original: {y.dtype}, Dummy: {dy.dtype}'
        assert x.dtype == dx.dtype, f'Original: {x.shape}, Dummy: {dx.shape}'
